 ...
 *     responses:
 *       200:
 *         description: List of books filtered by category
 *       400:
 ...
 * /books/sort:
 *   get:
 *     summary: Get sorted books by title or year
 *     parameters:
 *       - in: query
 *         name: by
 *         required: true
...
app.get('/books/sort', async (req, res) => {
  ...
  if (!by || !order || !['title', 'year'].includes(by) || !['asc', 'desc'].includes(order)) {
    return res.status(400).json({ message: 'Invalid sort parameters' });
  }

  try {
    const books = await bookModel.getSortedBooks(by, order);
    res.json(books);
  } catch (err) {
